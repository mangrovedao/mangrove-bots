...
contract E2E_swap {
  ...
  //
  //
  function get_random_zeroForOne_priceLimit(int _amountSpecified) internal view returns (uint160 sqrtPriceLimitX96) {
    ...
    sqrtPriceLimitX96 = minimumPrice
      + uint160((uint(_amountSpecified > 0 ? _amountSpecified : -_amountSpecified) % (currentPrice - minimumPrice)));
  }

  function get_random_oneForZero_priceLimit(int _amountSpecified) internal view returns (uint160 sqrtPriceLimitX96) {
    // help echidna a bit by calculating a valid sqrtPriceLimitX96 using the amount as random seed
    (uint160 currentPrice,,,,,,) = pool.slot0();
    uint160 maximumPrice = TickMath.MAX_SQRT_RATIO;
    sqrtPriceLimitX96 = currentPrice
      + uint160((uint(_amountSpecified > 0 ? _amountSpecified : -_amountSpecified) % (maximumPrice - currentPrice)));
  }
  ...
  function check_liquidityNet_invariant() internal {
    ...
    for (uint i = 0; i < usedTicks.length; i++) {
  ...
  function check_tick_feegrowth_invariant() internal {
    ...
    (,, uint tB_feeGrowthOutside0X128, uint tB_feeGrowthOutside1X128) = pool.ticks(tickBelow);
    (,, uint tA_feeGrowthOutside0X128, uint tA_feeGrowthOutside1X128) = pool.ticks(tickAbove);
  ...
  function check_swap_invariants(
    ...
    uint bal_buy_bfre,
    uint bal_buy_aftr,
    uint feegrowth_sell_bfre,
    uint feegrowth_sell_aftr,
    uint feegrowth_buy_bfre,
    uint feegrowth_buy_aftr
  ) internal {
    // prop #17
  ...
  function test_swap_exactIn_zeroForOne(uint128 _amount) public {
    ...
    require(token0.balanceOf(address(swapper)) >= uint(_amount));
    int _amountSpecified = int(_amount);
  ...
  function test_swap_exactIn_oneForZero(uint128 _amount) public {
    ...
    require(token1.balanceOf(address(swapper)) >= uint(_amount));
    int _amountSpecified = int(_amount);
