...
contract SetupToken {
  ...
  function mintTo(address _recipient, uint _amount) public {
...
contract SetupTokens {
  ...
  function mintTo(uint _tokenIdx, address _recipient, uint _amount) public {
...
contract UniswapMinter {
  ...
  function uniswapV3MintCallback(uint amount0Owed, uint amount1Owed, bytes calldata data) external {
...
contract UniswapSwapper {
  ...
  function uniswapV3SwapCallback(int amount0Delta, int amount1Delta, bytes calldata data) external {
    if (amount0Delta > 0) token0.transfer(address(pool), uint(amount0Delta));
    if (amount1Delta > 0) token1.transfer(address(pool), uint(amount1Delta));
  }
  ...
  function doSwap(bool _zeroForOne, int _amountSpecified, uint160 _sqrtPriceLimitX96)
    public
