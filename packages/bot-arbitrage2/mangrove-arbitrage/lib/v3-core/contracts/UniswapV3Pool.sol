...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
  ...
  /// @inheritdoc IUniswapV3PoolState
  uint public override feeGrowthGlobal0X128;
  /// @inheritdoc IUniswapV3PoolState
  uint public override feeGrowthGlobal1X128;
  ...
  /// @inheritdoc IUniswapV3PoolState
  mapping(int16 => uint) public override tickBitmap;
  /// @inheritdoc IUniswapV3PoolState
  ...
  /// check
  function balance0() private view returns (uint) {
  ...
  function _modifyPosition(ModifyPositionParams memory params)
    private
    noDelegateCall
    returns (Position.Info storage position, int amount0, int amount1)
  {
    ...
    returns (Position.Info storage position)
  {
    ...
    uint _feeGrowthGlobal0X128 = feeGrowthGlobal0X128; // SLOAD for gas optimization
    uint _feeGrowthGlobal1X128 = feeGrowthGlobal1X128; // SLOAD for gas optimization
    ...
    (uint feeGrowthInside0X128, uint feeGrowthInside1X128) =
  ...
  {
    require(amount > 0);
    (, int amount0Int, int amount1Int) = _modifyPosition(
      ModifyPositionParams({
        owner: recipient,
        tickLower: tickLower,
        tickUpper: tickUpper,
        liquidityDelta: int(amount).toInt128()
      })
    );
  ...
  function burn(int24 tickLower, int24 tickUpper, uint128 amount)
    ...
    returns (uint amount0, uint amount1)
  {
    (Position.Info storage position, int amount0Int, int amount1Int) = _modifyPosition(
      ModifyPositionParams({
        owner: msg.sender,
        tickLower: tickLower,
        tickUpper: tickUpper,
    ...
    returns (int amount0, int amount1)
  {
    ...
    if (zeroForOne) {
      if (amount1 < 0) TransferHelper.safeTransfer(token1, recipient, uint(-amount1));

      uint balance0Before = balance0();
      IUniswapV3SwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
      require(balance0Before.add(uint(amount0)) <= balance0(), "IIA");
    } else {
      if (amount0 < 0) TransferHelper.safeTransfer(token0, recipient, uint(-amount0));

      uint balance1Before = balance1();
      IUniswapV3SwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
      require(balance1Before.add(uint(amount1)) <= balance1(), "IIA");
    }
  ...
  /// @inheritdoc IUniswapV3PoolActions
  function flash(address recipient, uint amount0, uint amount1, bytes calldata data)
    external
    override
    lock
    noDelegateCall
  {
    ...
    uint balance0After = balance0();
    uint balance1After = balance1();
