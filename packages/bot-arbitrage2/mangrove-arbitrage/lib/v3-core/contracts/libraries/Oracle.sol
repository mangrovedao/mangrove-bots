...
library Oracle {
  ...
  {
    self[0] =
      Observation({blockTimestamp: time, tickCumulative: 0, secondsPerLiquidityCumulativeX128: 0, initialized: true});
    return (1, 1);
  }
  ...
  function lte(uint32 time, uint32 a, uint32 b) private pure returns (bool) {
    ...
    uint aAdjusted = a > time ? a : a + 2 ** 32;
    uint bAdjusted = b > time ? b : b + 2 ** 32;
    ...
    if (target == beforeOrAt.blockTimestamp) {
      ...
      return (
        ...
        beforeOrAt.secondsPerLiquidityCumulativeX128
          + uint160(
            (
              uint(atOrAfter.secondsPerLiquidityCumulativeX128 - beforeOrAt.secondsPerLiquidityCumulativeX128)
                * targetDelta
