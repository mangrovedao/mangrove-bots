...
contract TestUniswapV3Router is IUniswapV3SwapCallback {
  ...
  function swapForExact0Multi(address recipient, address poolInput, address poolOutput, uint amount0Out) external {
  ...
  function swapForExact1Multi(address recipient, address poolInput, address poolOutput, uint amount1Out) external {
    address[] memory pools = new address[](1);
    pools[0] = poolInput;
    IUniswapV3Pool(poolOutput).swap(
  ...
  function uniswapV3SwapCallback(int amount0Delta, int amount1Delta, bytes calldata data) public override {
    ...
    if (pools.length == 1) {
      ...
      int amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;
      ...
      if (amount0Delta > 0) {
        IERC20Minimal(IUniswapV3Pool(msg.sender).token0()).transferFrom(payer, msg.sender, uint(amount0Delta));
      } else {
        IERC20Minimal(IUniswapV3Pool(msg.sender).token1()).transferFrom(payer, msg.sender, uint(amount1Delta));
      }
    }
  }
}
