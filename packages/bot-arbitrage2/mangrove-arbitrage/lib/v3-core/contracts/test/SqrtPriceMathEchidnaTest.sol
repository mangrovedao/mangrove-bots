...
contract SqrtPriceMathEchidnaTest {
  function mulDivRoundingUpInvariants(uint x, uint y, uint z) external pure {
    require(z > 0);
    uint notRoundedUp = FullMath.mulDiv(x, y, z);
    uint roundedUp = FullMath.mulDivRoundingUp(x, y, z);
  ...
  function getNextSqrtPriceFromInputInvariants(uint160 sqrtP, uint128 liquidity, uint amountIn, bool zeroForOne)
    external
    pure
  {
  ...
  function getNextSqrtPriceFromOutputInvariants(uint160 sqrtP, uint128 liquidity, uint amountOut, bool zeroForOne)
    external
    pure
  {
  ...
  function getNextSqrtPriceFromAmount0RoundingUpInvariants(uint160 sqrtPX96, uint128 liquidity, uint amount, bool add)
    external
    pure
  {
    require(sqrtPX96 > 0);
  ...
  function getNextSqrtPriceFromAmount1RoundingDownInvariants(uint160 sqrtPX96, uint128 liquidity, uint amount, bool add)
    external
    pure
  {
    require(sqrtPX96 > 0);
  ...
  function getAmount0DeltaInvariants(uint160 sqrtP, uint160 sqrtQ, uint128 liquidity) external pure {
    ...
    uint amount0Down = SqrtPriceMath.getAmount0Delta(sqrtQ, sqrtP, liquidity, false);
    assert(amount0Down == SqrtPriceMath.getAmount0Delta(sqrtP, sqrtQ, liquidity, false));

    uint amount0Up = SqrtPriceMath.getAmount0Delta(sqrtQ, sqrtP, liquidity, true);
  ...
  function getAmount0DeltaEquivalency(uint160 sqrtP, uint160 sqrtQ, uint128 liquidity, bool roundUp) external pure {
    ...
    uint numerator1 = uint(liquidity) << FixedPoint96.RESOLUTION;
    uint numerator2 = sqrtP - sqrtQ;
    uint denominator = uint(sqrtP) * sqrtQ;

    uint safeResult = roundUp
      ? FullMath.mulDivRoundingUp(numerator1, numerator2, denominator)
      : FullMath.mulDiv(numerator1, numerator2, denominator);
    uint fullResult = SqrtPriceMath.getAmount0Delta(sqrtQ, sqrtP, liquidity, roundUp);
  ...
  function getAmount1DeltaInvariants(uint160 sqrtP, uint160 sqrtQ, uint128 liquidity) external pure {
    ...
    uint amount1Down = SqrtPriceMath.getAmount1Delta(sqrtP, sqrtQ, liquidity, false);
    assert(amount1Down == SqrtPriceMath.getAmount1Delta(sqrtQ, sqrtP, liquidity, false));

    uint amount1Up = SqrtPriceMath.getAmount1Delta(sqrtP, sqrtQ, liquidity, true);
  ...
  function getAmount0DeltaSignedInvariants(uint160 sqrtP, uint160 sqrtQ, int128 liquidity) external pure {
    ...
    int amount0 = SqrtPriceMath.getAmount0Delta(sqrtQ, sqrtP, liquidity);
  ...
  function getAmount1DeltaSignedInvariants(uint160 sqrtP, uint160 sqrtQ, int128 liquidity) external pure {
    ...
    int amount1 = SqrtPriceMath.getAmount1Delta(sqrtP, sqrtQ, liquidity);
  ...
  function getOutOfRangeMintInvariants(uint160 sqrtA, uint160 sqrtB, int128 liquidity) external pure {
    ...
    int amount0 = SqrtPriceMath.getAmount0Delta(sqrtA, sqrtB, liquidity);
    int amount1 = SqrtPriceMath.getAmount1Delta(sqrtA, sqrtB, liquidity);
    ...
    pure
  {
    ...
    int amount0 = SqrtPriceMath.getAmount0Delta(sqrtCurrent, sqrtUpper, liquidity);
    int amount1 = SqrtPriceMath.getAmount1Delta(sqrtLower, sqrtCurrent, liquidity);
