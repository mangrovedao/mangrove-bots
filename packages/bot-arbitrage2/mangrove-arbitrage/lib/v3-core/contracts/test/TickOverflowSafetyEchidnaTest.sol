...
contract TickOverflowSafetyEchidnaTest {
  ...
  uint private feeGrowthGlobal0X128 = type(uint).max / 2;
  uint private feeGrowthGlobal1X128 = type(uint).max / 2;
  // how much total growth has happened, this cannot overflow
  uint private totalGrowth0 = 0;
  uint private totalGrowth1 = 0;

  function increaseFeeGrowthGlobal0X128(uint amount) external {
    require(totalGrowth0 + amount > totalGrowth0); // overflow check
    feeGrowthGlobal0X128 += amount; // overflow desired
    totalGrowth0 += amount;
  }

  function increaseFeeGrowthGlobal1X128(uint amount) external {
